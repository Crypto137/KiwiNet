namespace KiwiNet.Core.Network
{
    ...
    {
        private const byte DisconnectionId = 0;
        ...
        public static bool DeserializeAllPackets(NetworkConnection connection, List<PacketSerializer> serializers)
        {
            ...
            if (connection.IsConnected == false)
            {
                foreach (PacketSerializer serializer in serializers)
                    serializer.TryDeserializePacket(DisconnectionId, connection);
            }
        ...
        {
            byte packetId = connection.Read<byte>();
        ...
        /// </remarks>
        public virtual bool TryDeserializePacket(byte packetId, NetworkConnection connection)
        {
            if (packetId == DisconnectionId)
            {
        ...
        protected abstract Packet ConstructAndDeserializePacket(byte packetId, NetworkConnection connection);
    }
}
